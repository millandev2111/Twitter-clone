...
export function AuthButton ({session} : {session: Session | null}) {
    ...
    const handleSignIn = async () => {
        await supabase.auth.signInWithOAuth({
            provider: 'github',
            options: {
                redirectTo: 'http://localhost:3000/auth/callback' //nos devuelve las credenciales del usuario
            }
        })
    }
    ...
    return (
        <header>
            ...
            <svg className="size-4 me-2" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
